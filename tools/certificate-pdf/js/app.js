...
let ready = null;
/* A build is in flight. While it is, nothing else may touch the status, the
   result, or the inputs: the build owns them until it finishes. Without this,
   changing the file selection (or the requirement) mid-build wiped the progress
   line, re-enabled Build for a second concurrent run, and left the user looking
   at an empty panel with no result and no error. */
let building = false;
...
async function build() {
  if (building || !picked.length) return;

  // Snapshot the inputs for THIS build. `picked` is reassigned whenever the file
  // selection changes, so reading it live can mix two selections into one build.
  ...
  // The inputs are disabled as well, so the selection cannot change underneath a
  // running build. Everything is restored in the finally below.
  els.fileInput.disabled = true;
  els.requirement.disabled = true;
  els.maxKB.disabled = true;
  clearResult();
  clearAttempts();
  ...
  try {
    ...
    if (bitmaps.length < files.length) {
      showStatus('', `Using ${bitmaps.length} of ${files.length} files`,
        `${files.length - bitmaps.length} could not be opened and were skipped.`);
    } else {
    ...
    els.requirement.disabled = false;
    els.maxKB.disabled = false;
...
function init() {
  ...
  els.fileInput.addEventListener('change', (e) => {
    // A build owns the inputs while it runs: it reads its own snapshot of the
    // selection and reports a result for that snapshot. Accepting a new selection
    // now would leave the panel showing one set of files beside a result computed
    // from another — and it also wiped the progress line and re-enabled Build for
    // a second concurrent run. The input is disabled during a build, so a real
    // user cannot reach this; the guard is here because a change event can still
    // be fired programmatically, which is exactly how the test drives it.
    if (building) return;
